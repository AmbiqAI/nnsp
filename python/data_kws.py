...
from nnsp_pack import tfrecord_converter_kws
...
def download_data():
    ...
    s3 = boto3.client('s3')
    audio_lists = [
        'data/test_files_galaxy.csv',
        'data/train_files_galaxy.csv',
        'data/noise_list.csv',
        'data/test_garb.csv',
        'data/train_garb.csv'
    ]
...
class FeatMultiProcsClass(multiprocessing.Process):
    ...
    def convert_tfrecord(
        ...
        for i, fname in enumerate(fnames):
            ...
            wavpath = sps[0]
            stime = int(sps[1])         # start time
            etime = int(sps[2])         # end time
            ...
            if success:
                ...
                try:
                    ...
                    tfrecord_converter_kws.make_tfrecord(
                        tfrecord,
                        feat,
...
def main(args):
    ...
    if not DEBUG:
        for train_set in train_sets:
            with open(f'data/{train_set}_tfrecords_kws.csv', 'w') as file: # pylint: disable=unspecified-encoding
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        '-t',
        '--train_dataset_path',
        default = 'data/train_files_galaxy.csv',
        help    = 'path to train data file')

    argparser.add_argument(
        '--test_dataset_path',
        default = 'data/test_files_galaxy.csv',
