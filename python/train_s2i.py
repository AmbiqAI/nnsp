...
from nnsp_pack.tfrecord_converter_s2i import tfrecords_pipeline
from nnsp_pack.loss_functions import cross_entropy
from nnsp_pack.converter_fix_point import fakefix_tf
from nnsp_pack.calculate_feat_stats import feat_stats_estimator
...
@tf.function
def train_kernel(   feat, mask, intent, slot0, slot1, states, net,
                    optimizer,
    ...
    with tf.GradientTape() as tape:
        ...
        est_intent  = tf.nn.softmax(est[:,:,:dim_intent])
        est_slot0   = tf.nn.softmax(est[:,:,dim_intent: dim_intent+dim_slot])
        est_slot1   = tf.nn.softmax(est[:,:,dim_intent+dim_slot:])

        intent = tf.one_hot(intent, dim_intent, dtype=tf.float32)
        ave_loss0, steps = cross_entropy(
                            intent,
                            est_intent,
                            masking = mask)

        slot0 = tf.one_hot(slot0, dim_slot, dtype = tf.float32)
        ave_loss1, steps = cross_entropy(
                            slot0,
                            est_slot0,
                            masking = mask)

        slot1 = tf.one_hot(slot1, dim_slot, dtype=tf.float32)
        ave_loss2, steps = cross_entropy(
                            slot1,
                            est_slot1,
                            masking = mask)

        ave_loss = 0.5 * (ave_loss0 + 0.5 * (ave_loss1 + ave_loss2) )
    ...
    return est_intent, est_slot0, est_slot1, states, ave_loss, steps
...
def epoch_proc( net,
    ...
    for batch, data in enumerate(dataset):
        tf.print(f"\r {batch}/{int(np.ceil(len(fnames) / batchsize))}: ",
                        end = '')

        feats, masks, intent, slot0, slot1, _ = data
        ...
        shape_intents = [[0, batchsize - batchsize0], [0, steps_pad]]
        paddings_intent = tf.constant(shape_intents)
        ...
        intent = tf.pad(intent, paddings_intent, "CONSTANT")

        slot0 = tf.pad(slot0, paddings_intent, "CONSTANT")

        slot1 = tf.pad(slot1, paddings_intent, "CONSTANT")
        ...
        for i in range( int(np.round(total_steps / timesteps))):
            ...
            intent_part = intent[:,start_fr:end_fr:num_dnsampl]
            slot0_part  = slot0[:,start_fr:end_fr:num_dnsampl]
            slot1_part  = slot1[:,start_fr:end_fr:num_dnsampl]
            tmp = train_kernel(
                    feat_part,
                    mask_part,
                    intent_part,
            ...
            est_intent, _, _, states, ave_loss, steps = tmp
            est_intent = tf.math.argmax(est_intent, axis=2)

            net.update_cost_steps(ave_loss, steps)

            net.update_accuracy(
                tf.cast(intent_part, tf.int64),
                est_intent,
                mask_part,
                dim_intent)
...
def main(args):
    ...
    for epoch in range(epoch1_loaded, num_epoch):
        ...
        nn_infer.show_confusion_matrix(DIM_INTENT, logger)
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        description='Training script for s2i model')
    ...
    argparser.add_argument(
        '-st',
        '--train_list',
        default='data/train_tfrecords_s2i.csv',
        help='train_list')

    argparser.add_argument(
        '-ss',
        '--test_list',
        default='data/test_tfrecords_s2i.csv',
        help='test_list')
